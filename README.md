...
> ALLOW_ENV_KEY_FALLBACK=1
> ```
>
> `ALLOW_ENV_KEY_FALLBACK=1` を付けたときだけ環境変数のキーが使われます。
> **公開環境（Streamlit Community Cloud 等）ではこのフラグを設定しないでください。**
> 設定すると、サイドバーにキーを入力していない匿名の訪問者全員がアプリ所有者のキーで
> 実行できてしまい、課金枠の悪用につながります（`.env` は `.gitignore` 対象なのでコミットされません）。
